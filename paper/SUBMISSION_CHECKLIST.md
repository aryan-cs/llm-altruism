# Submission Checklist

This file tracks whether the current paper bundle clears the project's own
submission-readiness bar.

- [x] Multi-model, multi-provider audited results collected
- [x] Scarcity society and reputation society experiments completed
- [x] Multiple core precursor games completed
- [x] Cross-model pairings and self-play completed
- [x] Benchmark-recognition precursor battery completed
- [x] Prompt-susceptibility precursor battery completed
- [x] Statistical summaries with confidence intervals generated
- [x] Exact paired tests generated for the focal repeated-game contrasts
- [x] Manuscript-facing figures generated
- [x] Manuscript-facing tables generated
- [x] Payoff matrices included in the paper bundle
- [x] Exact prompt text included in the appendix
- [x] Bibliography-backed citations wired into the build
- [x] PDF section-numbering cleanup applied in the build
- [x] Literature review drafted in the paper bundle
- [x] Methods reproducible from code and configs
- [x] Threats-to-validity discussion written
- [x] Ethics and release framing written
- [x] Artifact index and reproduction commands prepared
- [x] Anonymous ICML-style submission PDF generated
- [ ] Canonical long-horizon ecology baseline completed on the multi-resource world
- [ ] Canonical long-horizon reputation or event-stress follow-on completed on the same world
- [ ] Manuscript society claims fully updated from the predecessor institutional bundle to the canonical ecology bundle

## Remaining Nice-To-Haves

- adapt the current ICML-style build to NeurIPS or ICLR if one of those is the
  final target venue
- run deeper institutional replications for tighter confidence intervals
- expand the model panel toward more frontier systems
