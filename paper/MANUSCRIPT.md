...
This paper is designed to be reproducible from code and artifacts rather than
from narrative description alone. Experiments are config-driven, prompts live
in versioned files, raw outputs are preserved in JSON or JSONL artifacts, and
paper figures and summaries are regenerated by dedicated scripts. The paper
bundle therefore points directly to result directories, pooled summaries,
figure manifests, and regression tests rather than relying on hand-copied
tables. The associated appendix lists the exact commands used to regenerate the
current manuscript-facing summaries and figures.

One caveat is that provider availability is date-specific. The repaired model
panel reported here was selected from models that were both accessible and
experiment-ready during the April 2026 runs. Future reruns may therefore need
to substitute models if provider catalogs, rate limits, or API behavior
change. We treat that as an external source of variance to document, not a
reason to obscure the original cohort.

The release posture should also avoid anthropomorphic overclaiming. These
results do not show that a model is intrinsically altruistic, selfish, moral,
or aligned in any deep sense. They show that behavior in social settings is
measurably shaped by game structure, prompt framing, benchmark recognition, and
institutional rules. Releasing prompts, logs, and code is therefore useful for
scientific scrutiny, but the surrounding paper should consistently frame the
findings as behavioral evidence under specified conditions rather than as
evidence of stable inner motives.
