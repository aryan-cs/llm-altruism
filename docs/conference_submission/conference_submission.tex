...
\label{sec:scope}
Prosocial Readiness Bench measures controlled responses, not intent or moral status. This open-model pilot is limited by related variants and mutable local tags; Part~0 labels that scored rationale or response without row-level judge provenance; fixed Part~1 order and unretained retries; hypothetical one-shot rather than realized interaction; one homogeneous Part~2 trajectory per model; and missing decoding controls. Model tags, decoding defaults, judge provenance, prompt order, and transition parameters are properties of the measured provider--model stack, not stable family estimates.
...
\section{Conclusion}
Safety Beyond Refusal packages traceable probes of harmful-request refusal, focal dilemma choices, and commons restraint. The pilot profile is partially coupled: refusal and restraint co-vary positively, while role-conditioned and model-specific gaps show that refusal is not a substitute for the full profile. This trace-level evaluation is neither a latent prosociality score nor a realistic society simulation. Confirmatory claims require broader coverage, quantitative judge validation, recorded decoding controls, repeated trajectories, and sensitivity analysis of the commons dynamics.
