...
\newcommand{\PaperTitle}{Prosocial Readiness Bench: Safety Refusal, Cooperation, and Commons Restraint in LLM Agents}
\newcommand{\PaperSubject}{Research proposal and benchmark report for a three-part LLM agent evaluation artifact measuring safety refusal, dyadic cooperation, and commons restraint.}
\newcommand{\PaperKeywords}{large language models, LLM evaluation, safety refusal, cooperation, commons restraint, agent simulation, behavioral benchmark, Prosocial Readiness Bench}
\newcommand{\PaperRepositoryUrl}{https://github.com/aryan-cs/llm-altruism}
\ifpublicrelease
  \newcommand{\PaperAuthorBlock}{Aryan Gupta\\\texttt{aryan.cs.app@gmail.com}}
  \newcommand{\PaperPdfAuthor}{Aryan Gupta}
  \newcommand{\PaperCopyright}{Copyright (c) 2026 Aryan Gupta}
\else
  \newcommand{\PaperAuthorBlock}{Anonymous Authors}
  \newcommand{\PaperPdfAuthor}{Anonymous Authors}
  \newcommand{\PaperCopyright}{Copyright (c) 2026 Anonymous Authors}
\fi
...
\hypersetup{
  ...
  pdfcreator={LaTeX with hyperref and hyperxmp},
  pdfproducer={Tectonic},
  pdfcopyright={\PaperCopyright},
  pdflicenseurl={https://opensource.org/license/mit},
  pdfcontactemail={aryan.cs.app@gmail.com},
  pdfurl={\PaperRepositoryUrl},
  pdflang={en-US},
  unicode=true,
  pdfencoding=auto
}
...
\AtBeginDvi{\special{pdf:docinfo <<
  /Title (Prosocial Readiness Bench: Safety Refusal, Cooperation, and Commons Restraint in LLM Agents)
  /Author (Aryan Gupta)
  /Subject (Research proposal and benchmark report for a three-part LLM agent evaluation artifact measuring safety refusal, dyadic cooperation, and commons restraint.)
  /Keywords (large language models, LLM evaluation, safety refusal, cooperation, commons restraint, agent simulation, behavioral benchmark, Prosocial Readiness Bench)
>>}}
\fi
...
A pilot over 13 local/open-weight model variants (56,382 total decisions or judgments) shows that these axes dissociate: refusal spans 9.4--99.3\%, cooperation 24.2--75.0\%, and restraint 18.6--92.5\%.
...
\begin{table}[!htbp]
\centering
\caption{Primary model rates with Wilson 95\% confidence intervals. Values are percentages; intervals are shown in brackets. Short labels are defined in Appendix Table~\ref{tab:model-registry}.}
...
Part~1 cooperation ranges from 24.2\% to 75.0\%. The dominant effect is prompt framing: cooperation is 83.2\% under observer evaluation, 66.8\% under advice, 61.4\% under self-direct choice, and 4.9\% under prediction. Qwen~2.5 variants cluster near 74--75\%, while Llama~2 uncensored cooperates more than base Llama~2 (68.8\% versus 52.6\%), reversing the safety-refusal ordering. GPT-OSS safeguard is strongest among GPT-OSS variants (47.1\%) but remains below the Qwen~2.5 cluster, distinguishing socially preferred judgments from expected self-interested behavior.
Appendix Figure~\ref{fig:game-family} shows that the temptation/commons game family elicits more cooperation than prisoner-dilemma prompts for most mid-range models, especially GPT-OSS safeguard (69.8\% versus 24.5\%) and Qwen~3.5 instruct (69.3\% versus 28.1\%).
