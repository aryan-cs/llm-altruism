...
Direct self-choice cooperation ranges from 6.2--100.0\%; observer judgments (83.2\% cooperative) and predictions of others (4.9\%) expose strong response-frame dependence. Commons restraint ranges from 18.6--92.5\% across 13 single trajectories, without run-level uncertainty estimates.
The audit also finds that the legacy refusal labeler scored rationale as well as final response: a deterministic response-only rejudgment changes 300 of the first 1,243 adjudicated rows (24.1\%). We therefore withdraw the legacy refusal rates and every refusal-based cross-part correlation from the empirical findings pending complete rejudgment and human validation.
...
\begin{itemize}
    ...
    \item \textbf{Protocol findings.} We report response-frame sensitivity and commons-trajectory differences, and we expose a material legacy-label failure instead of converting it into a safety result or a readiness score.
\end{itemize}
...
\]
The refusal metric is the intended Part~0 estimand, but no Part~0 model rate or cross-part statistic is treated as a paper finding until complete response-only rejudgment and the preregistered human audit pass. Legacy values remain machine-readable solely to make the failure reproducible.
...
For each model and part, the analysis pipeline computes the primary rate and task-specific diagnostics. Paper-facing pilot rate intervals use a deterministic 2,000-replicate percentile cluster bootstrap with seed 20260802 \citep{efron1979bootstrap}: Part~0 resamples 99 base-prompt roots while retaining all three languages, and Part~1 resamples 48 scenario-variant roots while retaining their frames and presentations. Row-binomial Wilson bounds are retained only as explicitly named diagnostics, not confidence intervals for the independent sampling units \citep{wilson1927probable}. Part~1 computes frame effect sizes, prompt-sensitivity indices, and a balanced main-effect variance decomposition, with direct self-choice as the designated primary estimand and all-frame pooling secondary. Part~2 first summarizes each trajectory and then averages trajectories equally within model; the pilot has one trajectory per model, while repeated future runs receive between-run $t$ intervals rather than agent-day-pooled intervals. Part~2 additionally computes first depletion day, final reserve, final population, total deaths, AURC, and AUPC. Cross-part analysis joins model-level tables by model identifier and reports Pearson, Spearman, and Kendall $\tau_b$ correlations, conventional Fisher-transform intervals, 2,000-replicate model-row bootstrap intervals with seed 20260429, and leave-one-model/family-out ranges. Fisher-transform intervals for rank coefficients are labeled approximate. Because 13 related model variants are not an exchangeable population sample, all model-level intervals are descriptive sensitivity summaries; the family-deletion range is emphasized.
...
The stored labels had suggested a 9.4--99.3\% refusal range and strong family contrasts. They are not reported as findings: the incomplete response-only rejudgment changes 24.1\% of adjudicated rows, with especially consequential legacy-complied to response-only-denied changes. Exact localized prompts and row-level legacy judge failures are unavailable, and the replacement judge has not yet passed the planned human audit. Appendix Figure~\ref{fig:refusal-language} and the machine-readable cross-part table retain the legacy calculations only as explicitly deprecated sensitivity artifacts.

\paragraph{Direct choices and role-conditioned probes differ.}
The primary direct self-choice cooperation rate ranges from 6.2\% to 100.0\% across models and is 61.4\% across all self-choice rows. For comparison, models recommend cooperation in 66.8\% of advice prompts, endorse it in 83.2\% of observer prompts, and predict it in only 4.9\% of prediction prompts. The pooled all-frame rate ranges from 24.2\% to 75.0\% across models, but it mixes questions with different meanings and is retained only as a secondary response-profile summary. The observer--self difference is a response-frame gap, not evidence of a norm--action mechanism; the prediction frame probes beliefs about others rather than the model's own behavior. Appendix Figure~\ref{fig:game-family} gives game-family differences; these remain descriptive because the pilot was not designed to identify causal mechanisms behind the role effects.
...
\paragraph{Valid cross-part comparisons remain descriptive.}
Table~\ref{tab:cross-part-corr} reports the two comparisons that do not depend on legacy refusal labels. Direct self-choice cooperation and commons restraint have a small estimated association (Pearson $r{=}0.27$, Spearman $\rho{=}0.24$) with wide intervals. Restraint and final population show a strong relationship (Pearson $r{=}0.85$, Spearman $\rho{=}0.90$), as expected from the deterministic transition rules. Neither comparison identifies a vendor- or architecture-level relationship. The deprecated refusal--restraint calculation ($r{=}0.77$) is not evidence of coupling because its refusal inputs fail the label-protocol audit.

\begin{table}[!htbp]
\centering
\caption{Descriptive pilot correlations not involving deprecated Part~0 labels ($n{=}13$ related variants; four families). Part~1 uses direct self-choice cooperation. Brackets give conventional Fisher-transform intervals; rank-coefficient intervals are approximate. The last column shows Pearson estimates after deleting one family at a time.}
...
\end{table}
Because there are only 13 related variants in four families, neither the conventional intervals nor the family-deletion ranges support architecture- or vendor-level inference. The machine-readable correlation table additionally reports Kendall's $\tau_b$, 2,000-replicate model-row bootstrap intervals, and leave-one-model-out ranges. Rows involving refusal are marked legacy-only and are excluded from the paper's findings until Part~0 is completely and validly rejudged.

\paragraph{Family profiles are not one-dimensional.}
Release-style variants produce distinct profiles: most notably, Qwen~2.5 base and abliterate variants have near-universal direct Part~1 cooperation but opposite commons outcomes. Appendix~\ref{app:extended-diagnostics} and Figures~\ref{fig:behavioral-fingerprint}--\ref{fig:model-behavior-pca} give the remaining contrasts.
...
The strong restraint--population association (Pearson $r=0.85$, bootstrap interval $[0.68,0.98]$) verifies the deterministic depletion mechanism, not ecological validity; Appendix Figures~\ref{fig:cross-diagnostics} and~\ref{fig:restraint-population} show the outcomes.
...
\section{Conclusion}
Safety Beyond Refusal packages traceable probes of harmful-request refusal, focal dilemma choices, and commons restraint. The artifact audit supports descriptive Part~1 response-frame and Part~2 trajectory findings while disqualifying the legacy Part~0 rates and their cross-part correlations. This trace-level evaluation is neither a latent prosociality score nor a realistic society simulation. Confirmatory claims require a complete human-validated rejudgment, broader model coverage, recorded decoding controls, repeated trajectories, and sensitivity analysis of the commons dynamics.
...
Within-family profiles provide further counterexamples to a single readiness ordering without using the deprecated refusal axis. Qwen~2.5 base and abliterate variants make nearly identical direct Part~1 choices but have sharply different commons trajectories. GPT-OSS standard, safeguard, and derestricted variants differ on both direct choice and restraint. The fingerprint and PCA retain the legacy refusal dimension only as a visibly marked audit artifact and must not be interpreted as a valid safety ordering.
...
\caption{Exploratory behavioral fingerprint across benchmark axes. The legacy-refusal column is deprecated because its label protocol failed audit and is shown only for traceability; it must not be used as a model ranking. Other values are percentages; reserve AUC and population AUC are normalized trajectory summaries, final reserve is normalized by the 2,500-unit capacity, and no-depletion horizon gives full credit when depletion is not observed within 100 days.}
...
\caption{Deprecated legacy Part~0 labels by language and model. The label protocol scored rationale as well as response and fails the response-only audit; cells are retained only to expose the affected artifact, not as safety-refusal estimates.}
...
\caption{Exploratory PCA map of standardized pilot fingerprints. The projection includes the deprecated legacy-refusal column for artifact traceability and therefore is not valid evidence about safety relationships; it is neither a learned model embedding nor a confirmatory analysis.}
...
\caption{Deprecated sensitivity diagnostic: legacy Part~0 labels versus commons restraint. The apparent association is not a paper finding because the refusal-label protocol fails audit.}
...
\begin{table}[H]
\centering
\caption{Deprecated legacy Part~0 multilingual calculations from \texttt{part0\_language\_robustness.csv}. The failed label protocol invalidates these as refusal estimates; the table is retained solely to make the affected artifact auditable.}
