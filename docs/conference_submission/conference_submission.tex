...
\begin{itemize}
    ...
    \item \textbf{Audited open-model pilot.} We trace 56,382 stored rows to 99 multilingual prompt roots, 48 Part~1 scenario roots per model, and 13 Part~2 trajectories; clustered intervals respect the prompt roots, and no agent-day interval is presented as run uncertainty.
...
\end{tikzpicture}}
\caption{Prosocial Readiness Bench pipeline. Each task family produces structured traces, metrics, and a locally checked candidate package.}
\label{fig:overview}
\end{figure}
...
\]
Part 2 also reports final reserve, first depletion day, final population, total deaths, survival, and normalized area under the reserve curve (AURC) and population curve (AUPC):
\[
...
\begin{table}[!htbp]
\centering
\caption{Auditable pilot rates. Direct self-choice cooperation includes a 95\% interval clustered by the 48 scenario-variant roots, keeping presentations together. Commons restraint is a point estimate from one state-coupled trajectory per model, with no run-uncertainty interval. Legacy Part~0 rates are withdrawn rather than reproduced as findings. Values are percentages.}
\label{tab:primary-rate-ci}
\scriptsize
\resizebox{\linewidth}{!}{%
\begin{tabular}{lcc}
\toprule
Model & Direct self-choice coop. & Commons restraint \\
\midrule
...
These descriptive results over 13 related local/open-weight variants demonstrate benchmark behavior and generate hypotheses rather than estimate vendor or architecture effects.
...
Table~\ref{tab:cross-part-corr} reports the two comparisons supported by the audited Part~1 and Part~2 inputs. Direct self-choice cooperation and commons restraint have a small estimated association (Pearson $r{=}0.27$, Spearman $\rho{=}0.24$) with wide intervals. Restraint and final population show a strong relationship (Pearson $r{=}0.85$, Spearman $\rho{=}0.90$), as expected from the deterministic transition rules. Neither comparison identifies a vendor- or architecture-level relationship.

\begin{table}[!htbp]
\centering
\caption{Descriptive pilot associations from the audited Part~1 and Part~2 inputs ($n{=}13$ related variants; four families). Part~1 uses direct self-choice cooperation. Brackets give conventional Fisher-transform intervals; rank-coefficient intervals are approximate. The last column shows Pearson estimates after deleting one family at a time.}
...
The strong restraint--population association (Pearson $r=0.85$, finite-panel resampling interval $[0.68,0.98]$) verifies the deterministic depletion mechanism, not ecological validity; Appendix Figure~\ref{fig:restraint-population} shows the outcomes.
...
\caption{Mechanism diagnostic: commons restraint versus final simulated population. Restraint tracks survival in the repeated commons environment.}
