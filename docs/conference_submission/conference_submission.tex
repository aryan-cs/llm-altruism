...
For each model and part, the analysis pipeline computes the primary rate and task-specific diagnostics. Paper-facing Part~1 intervals use a deterministic 2,000-replicate percentile cluster bootstrap with seed 20260802, resampling 48 scenario-variant roots while retaining their frames and presentations \citep{efron1979bootstrap}. Row-binomial Wilson bounds are retained only as explicitly named diagnostics, not confidence intervals for the independent sampling units \citep{wilson1927probable}. Part~1 computes calibration contrasts, prompt-sensitivity indices, and a balanced main-effect variance decomposition, with direct self-choice as the designated primary estimand and all-frame pooling secondary. Part~2 first summarizes each trajectory and then averages trajectories equally within model; the pilot has one trajectory per model, while repeated future runs receive between-run $t$ intervals rather than agent-day-pooled intervals. Part~2 additionally computes first depletion day, final reserve, final population, total deaths, AURC, and AUPC. Model-level association analysis joins the valid tables and reports Pearson, Spearman, and Kendall $\tau_b$ correlations, conventional Fisher-transform intervals, 2,000-replicate model-row bootstrap intervals with seed 20260429, and leave-one-model/family-out ranges. Fisher-transform intervals for rank coefficients are labeled approximate. Because 13 related model variants are not an exchangeable population sample, all model-level intervals are descriptive sensitivity summaries; the family-deletion range is emphasized.
...
The strong restraint--population association (Pearson $r=0.85$, bootstrap interval $[0.68,0.98]$) verifies the deterministic depletion mechanism, not ecological validity; Appendix Figure~\ref{fig:restraint-population} shows the outcomes.
...
\setlength{\parsep}{0pt}
    ...
    \item Croissant metadata: \path{data/analysis/croissant_metadata.json}. Croissant 1.1/RAI 1.0 validation and real-row loading pass for all 11 record sets; a stable anonymous dataset URL must be added with the generator's \texttt{--dataset-url} option before public hosting.
